<template>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        <article v-for="post in posts" :key="post.id"
            ...
            <img v-if="post.cover" class="rounded h-40 object-cover" :src="`https://static.dublecode.ru${post.cover.url}`" alt="">
            <img v-else class="rounded h-40 object-cover" src="/no-image.jpeg">
            <h3 class="text-2xl text-orange-100 flex-auto">{{ post.title }}</h3>
            <NuxtLink to="/"
                class="w-full flex items-center justify-center py-2
                    text-orange-500 bg-orange-100 hover:bg-orange-300 hover:text-orange-100
